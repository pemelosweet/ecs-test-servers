...
// 开关 config.CAPTCHA.ENABLED 在 config.js，恢复时置 true 并同步前端 RegisterPage 的 GRAPHIC_CAPTCHA_ENABLED
const crypto = require('crypto');
const config = require('../config');

async function verifyGraphicCaptcha({ lot_number, captcha_output, pass_token, gen_time }) {
  const { APP_ID, APP_KEY, VALIDATE_URL } = config.CAPTCHA;
  if (!APP_ID || !APP_KEY) {
    throw new Parse.Error(
      Parse.Error.VALIDATION_ERROR,
      '图形认证未配置：.env 缺少 GRAPHIC_CAPTCHA_APP_KEY'
    );
  }
  if (!lot_number || !captcha_output || !pass_token || !gen_time) return false;

  const sign_token = crypto.createHmac('sha256', APP_KEY).update(lot_number).digest('hex');
  try {
    const res = await fetch(`${VALIDATE_URL}?captcha_id=${APP_ID}`, {
      method: 'POST',
...
module.exports = { verifyGraphicCaptcha };
